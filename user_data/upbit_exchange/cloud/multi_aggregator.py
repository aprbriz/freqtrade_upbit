...
class MultiAggregator:
    ...
    def shutdown(self):
        ...
        try:
            ...
            # 틱 기반 aggregator partial 폐기
            logger.info("틱 기반 aggregator partial 폐기 중...")
            for tick_aggr in self.tick_aggrs:
                tick_aggr.discard_pending()
    ...
    def reset_generation(self):
        """
        재연결 시 세션 전환 처리 (partial 폐기)
        """
        for aggr in self.time_aggrs:
            aggr.reset_state()
        for tick_aggr in self.tick_aggrs:
            tick_aggr.reset_state()
        for derived in self.derived_aggrs:
            derived.reset_state()
        with self.derived_lock:
            self.derived_last_ts.clear()
    ...
    def _update_derived_from_1s(self, pair: str, ts_ms: int, candle: dict):
        ...
        with self.derived_lock:
            ...
            self.derived_last_ts[pair] = max(ts_ms, last_ts or ts_ms)
        for derived in self.derived_aggrs:
            try:
